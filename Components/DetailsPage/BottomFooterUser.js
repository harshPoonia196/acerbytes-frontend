import { BottomNavigation, BottomNavigationAction, Box, Card, Fab, Menu, MenuItem } from '@mui/material'
                ...
                >
                    {isLogged ?
                        <BottomNavigationAction sx={{ flex: '0 1 auto', minWidth: 0 }}
                            label="Like" icon={propertyData?.isFav ? (
                                <ThumbUpIcon sx={{ color: colors.BLUE }} />
                            ) : (
                                <ThumbUpOffAltIcon />
                            )} onClick={handlefavClick} /> :
                        <BottomNavigationAction sx={{ flex: '0 1 auto', minWidth: 0 }}
                            label="Like" onClick={() => router.push(listOfPages.login)}
                            icon={<ThumbUpOffAltIcon />} />
                    }
                    <BottomNavigationAction sx={{ flex: '0 1 auto', minWidth: 0 }}
                        label="Share" onClick={handleOpenEnquiryForm}
                        icon={<ReplyIcon sx={{ transform: "scaleX(-1)" }} />} />
                    <BottomNavigationAction sx={{ flex: '0 1 auto', minWidth: 0 }}
                        label="Contact" onClick={handleOpenEnquiryForm}
                        icon={<WhatsAppIcon />} />
                    <BottomNavigationAction sx={{ flex: '0 1 auto', minWidth: 0 }}
                        label="Enquire" icon={<AssignmentIcon />} onClick={handleOpenEnquiryForm} />
                    <Menu
                        id="basic-menu"
                        anchorEl={anchorEl}
                        open={open}
