  ...
  const OfferCard = ({offer}) => {
    return(
        <Grid container className='offers'>
            <Grid item xs={12} sm={6} className='leftCard' sx={{padding:0, background: "#c1c1c1"}}>
                <Box sx={{background: "transparent", p: 4, boxShadow: "1px 2px 6px -2px gainsboro!important", height: "100%"}}>
                    <Typography variant='h2' fontWeight="bold">{offer.id}</Typography>
                    <Typography variant="h4" fontWeight="bold">{offer.title}</Typography>
                </Box>
            </Grid>
            <Grid item xs={12} sm={6} className='rightCard' sx={{padding:0, background: '#e6e6e6'}}>
                <Box sx={{background: "transparent", padding: 4, boxShadow: "1px 2px 6px -2px gainsboro!important", height: "100%"}}>
