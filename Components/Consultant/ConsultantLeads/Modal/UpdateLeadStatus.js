...
function UpdateLeadStatus({
  ...
  useEffect(() => {
    if (isEdit) {
      ...
      if (type === NOTES_TYPE.COMPLETED) {
        setFormValue({
          completed: { note, name, status, time },
          next: {},
          userId: userId,
        });
      } else {
  ...
  const getList = async () => {
    ...
    onSave = async () => {
      const {
          completed: { time, status, note } = {},
          next: { time: timeNext, status: statusNext, note: noteNext } = {},
          userId = "",
        } = formValue,
        {
          time: timeError = false,
          status: statusError = false,
          note: noteError = false,
        } = getValidationError(
          { key1: "time", value1: time },
          { key2: "status", value2: status },
          { key3: "note", value3: note }
        ),
        {
          statusNext: statusNextError = false,
          timeNext: timeNextError = false,
          noteNext: noteNextError = false,
        } = getValidationError(
          { key1: "timeNext", value1: timeNext },
          { key2: "statusNext", value2: statusNext },
          { key3: "noteNext", value3: noteNext }
        );
      ...
      if (statusNextError && timeNextError && noteNextError) {
        ...
        errorsNext = {
          statusNext: statusNextError,
          timeNext: timeNextError,
          noteNext: noteNextError,
        };
      }
      ...
      for (const key in errors) {
        if (Object.hasOwnProperty.call(errors, key)) {
          ...
          if (element) {
            isError = true;
            showTostMessages("Please fill required fields", "error");
            break;
          }
        }
      }
      ...
      if (!userId) {
        isError = true;
        showTostMessages("Please select customer", "error");
      } else if (
        !(time || status || note || timeNext || statusNext || noteNext)
      ) {
        isError = true;
        showTostMessages("Please fill completed or next note fields", "error");
      }

      if (!isError) {
        const data = { userId };
        if (userId) {
          const { isNonLogin, label } = myLeadsCustomer.find(
            (res) => res.value === userId
          );
        ...
        try {
          ...
          if (response.status == 200) {
            showTostMessages(
              isEdit
                ? ToasterMessages.NOTE_UPDATED_SUCCESS
                : ToasterMessages.NOTE_CREATED_SUCCESS,
              "success"
            );
            handlePopClose();
            getNotesList();
    ...
    >
      <DialogTitle onClose={handlePopClose}>
        <Typography variant="h4" sx={{ fontWeight: 700 }}>
          {isEdit
            ? `Edit Note for ${userDetails?.name?.firstName} ${userDetails?.name?.lastName}`
            : `Add Note for ${userDetails?.name?.firstName} ${userDetails?.name?.lastName}`}
        </Typography>
      </DialogTitle>
      <DialogContent>
        <Grid container spacing={2}>
          {
            <NewSelectTextFieldStructure
              ...
              isEdit={true}
              full
              list={myLeadsCustomer}
              value={formValue?.userId ?? null}
              handleChange={(e) => userChange(e)}
              error={errors.userId}
            />
          }
          ...
          {!isEdit || (isEdit && editData?.type === NOTES_TYPE.COMPLETED) ? (
            <>
              ...
              <Grid item xs={12} sm={6}>
                <SelectTextFields
                  label="Status"
                  value={formValue?.completed?.status ?? null}
                  list={NOTES_STATUS}
                  error={errors.status}
                  handleChange={(e) =>
                    handleChange(e, "status", NOTES_TYPE.COMPLETED)
                  }
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DemoContainer
                    sx={{ p: 0, overflow: "unset" }}
                    components={["DateTimePicker", "DateTimePicker"]}
                  >
                    <DateTimePicker
                      value={
                        formValue?.completed?.time
                          ? dayjs(formValue?.completed?.time)
                          : null
                      }
                      onChange={(date) =>
                        handleChange(date, "time", NOTES_TYPE.COMPLETED)
                      }
                      label="Time"
                      viewRenderers={{
              ...
              <InputField
                variant="outlined"
                value={formValue?.completed?.note ?? ""}
                multiline
                rows={2}
                label="Add note"
                error={errors.note}
                handleChange={(e) =>
                  handleChange(e, "note", NOTES_TYPE.COMPLETED)
                }
              />
            </>
          ) : null}
          ...
          {!isEdit || (isEdit && editData?.type === NOTES_TYPE.NEXT) ? (
            <>
              ...
              <Grid item xs={12} sm={6}>
                <SelectTextFields
                  value={formValue?.next?.status ?? ""}
                  label="Status"
                  list={NOTES_STATUS}
                  error={errors.statusNext}
                  handleChange={(e) =>
                    handleChange(e, "status", NOTES_TYPE.NEXT)
                  }
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  ...
                  >
                    <DateTimePicker
                      value={
                        formValue?.next?.time
                          ? dayjs(formValue?.next?.time)
                          : null
                      }
                      onChange={(date) =>
                        handleChange(date, "time", NOTES_TYPE.NEXT)
                      }
                      label="Time"
                      viewRenderers={{
                        hours: renderTimeViewClock,
              ...
              <InputField
                variant="outlined"
                value={formValue?.next?.note ?? ""}
                multiline
                rows={2}
                label="Add note"
                error={errors.noteNext}
                handleChange={(e) => handleChange(e, "note", NOTES_TYPE.NEXT)}
              />
            </>
          ) : null}
        </Grid>
